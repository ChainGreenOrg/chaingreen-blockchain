...
global.daemon_rpc_ws = `wss://${self_hostname}:55700`;
...
function loadConfig(net) {
  try {
    ...
    const config_root_dir =
      'CHAINGREEN_ROOT' in process.env
        ? process.env.CHAINGREEN_ROOT
        : path.join(os.homedir(), '.chaingreen', version);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 55700); // jshint ignore:line
