...
class TestSSL:
    ...
    async def test_farmer(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            farmer_server.chaingreen_ca_crt_path.read_bytes(), farmer_server.chaingreen_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
    ...
    async def test_full_node(self, wallet_node):
        ...
        generate_ca_signed_cert(
            full_node_server.chaingreen_ca_crt_path.read_bytes(),
            full_node_server.chaingreen_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
    ...
    async def test_wallet(self, wallet_node):
        ...
        generate_ca_signed_cert(
            wallet_server.chaingreen_ca_crt_path.read_bytes(), wallet_server.chaingreen_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
    ...
    async def test_harvester(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            harvester_server.chaingreen_ca_crt_path.read_bytes(),
            harvester_server.chaingreen_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
    ...
    async def test_introducer(self, introducer):
        ...
        # Create not authenticated cert
        pub_crt = introducer_server.chaingreen_ca_key_path.parent / "p2p.crt"
        pub_key = introducer_server.chaingreen_ca_key_path.parent / "p2p.key"
        generate_ca_signed_cert(
            introducer_server.chaingreen_ca_crt_path.read_bytes(),
            introducer_server.chaingreen_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
    ...
    async def test_timelord(self, timelord):
        ...
        generate_ca_signed_cert(
            timelord_server.chaingreen_ca_crt_path.read_bytes(),
            timelord_server.chaingreen_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
