...
from chaingreen.util.service_groups import validate_service
from chaingreen.util.setproctitle import setproctitle
from chaingreen.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Chaingreen.")
    quit()
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in chaingreen_init
