...
from chaingreen.util.ssl_check import DEFAULT_PERMISSIONS_CERT_FILE, DEFAULT_PERMISSIONS_KEY_FILE
...
def get_chaingreen_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "chaingreen_ca.crt")
    key = pkg_resources.resource_string(__name__, "chaingreen_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Chaingreen"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("chaingreen.org")]),
            critical=False,
        )
...
def main():
    return make_ca_cert(Path("./chaingreen_ca.crt"), Path("./chaingreen_ca.key"))
