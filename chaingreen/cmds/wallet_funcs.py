...
from chaingreen.cmds.units import units
from chaingreen.rpc.wallet_rpc_client import WalletRpcClient
from chaingreen.server.start_wallet import SERVICE_NAME
...
async def print_balances(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    for summary in summaries_response:
        ...
        if typ != "STANDARD_WALLET":
            ...
            print(
                f"   -Pending Total Balance: {balances['unconfirmed_wallet_balance']/units['chaingreen']} \
                {address_prefix} "
    ...
    if log_in_response["success"] is False:
        if log_in_response["error"] == "not_initialized":
            ...
            if "backup_path" in log_in_response:
                ...
                print(f"Backup file from backup.chaingreen.org downloaded and written to: {path}")
