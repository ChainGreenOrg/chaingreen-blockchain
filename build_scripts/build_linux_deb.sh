...
--appVersion=$CHAINGREEN_INSTALLER_VERSION --executable-name=chaingreen-blockchain
LAST_EXIT_CODE=$?
...
--arch "$PLATFORM" --options.version $CHAINGREEN_INSTALLER_VERSION --options.bin chaingreen-blockchain --options.name chaingreen-blockchain
LAST_EXIT_CODE=$?
if [ "$LAST_EXIT_CODE" -ne 0 ]; then
